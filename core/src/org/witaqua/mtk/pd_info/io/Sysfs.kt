...
package org.witaqua.mtk.pd_info.io
...
object RootSysfs : Sysfs {
    ...
    override fun read(paths: Collection<String>): Map<String, String> {
         ...
         * "source [sink]", "Source attached (default current)" - survive it
         * intact.
         *
         * A record is a line, so the newlines inside a value have to become
         * something else on the way through. caps_info is the reason: it is
         * the one attribute here that is a list rather than a figure, and
         * deleting its line breaks would run every object together into one
         * unparseable line. The unit separator is what they travel as, and
         * [UNIT_SEPARATOR] turns them back.
         */
        val script = paths.joinToString("\n") { path ->
            val quoted = "'${path.shellSafe()}'"
            "[ -r $quoted ] && printf '%s\\t%s\\n' $quoted " +
                "\"\$(cat $quoted 2>/dev/null | tr -d '\\r' | tr '\\n' '\\037')\""
        } +
            /*
        ...
        return shell(script)
            ...
            ?.mapNotNull { line ->
                ...
                if (tab <= 0) {
                    ...
                    line.substring(0, tab) to
                        line.substring(tab + 1).replace(UNIT_SEPARATOR, '\n').trim()
                }
            }
    ...
    /* What a newline inside a value travels as. Nothing in /sys uses it. */
